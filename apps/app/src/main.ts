...
(async function bootstrap() {
  ...
  app.setBaseViewsDir(join(process.env.PROJECT_ROOT, "apps/app/views"));
  ...
  writeFileSync(join(process.env.PROJECT_ROOT, "resources/docs/app.json"), JSON.stringify(document));
  if (enableSwagger) SwaggerModule.setup("docs", app, document);

  await app.listen(port);
  return app;
})().then(async (app) => {
  new CommonLogger("NestApplication").verbose(`App Store app is running on ${await app.getUrl()}`);
});
